...
# open variant table
variant_table = pd.read_csv(args.variant_table, sep='\t', 
                            index_col=['CHROM', 'POS'])
# open bam file
...
new_cols = ['GT', 'DP', 'AD', 'AO', 'RO', 'TAMP', 'SAMP', 'AMPS', 'VAF']
for new_col in new_cols:
  variant_table[new_col] = None

for row_i, row in variant_table.iterrows():
    numvars += 1

    # Variant table should only have one allele per line, but fake 
    # a list of alts to avoid having to modify DS original code
    # Also create a placeholder object for rec/vline that would have 
    # come from the VCF entries
    # handle multiallelic entries in the VCF (and split them)
    for alt in [row['ALT']]:
        rec = Object()
        rec.contig = row.name[0]
        rec.pos = row.name[1]
        rec.ref = row['REF']
        rec.alts = [ alt ]
        ...
        if len(rec.ref) == len(rec.alts[0]):
            ...
            altseqs = []
            window_rows = variant_table.loc[(rec.contig, slice(rec.pos-indelwindow, rec.pos+indelwindow))]
            for window_row_i, window_row in window_rows.iterrows():
                var = Object()
                var.contig = window_row.name[0]
                var.pos = window_row.name[1]
                var.ref = window_row['REF']
                var.alts = [ window_row['ALT'] ]
            #for var_i, var in vcffile.fetch(contig=rec.contig,start=rec.pos-indelwindow,end=rec.pos+indelwindow,reopen=True):
        ...
        # print("\t".join(line))
        variant_table.loc[row_i, 'GT'] = mygt
        variant_table.loc[row_i, 'DP'] = ro+ao
        variant_table.loc[row_i, 'AD'] = (ro, ao)
        variant_table.loc[row_i, 'AO'] = ao
        variant_table.loc[row_i, 'RO'] = ro
        variant_table.loc[row_i, 'TAMP'] = int(totalamplicons)
        variant_table.loc[row_i, 'SAMP'] = int(supportingamplicons)
        variant_table.loc[row_i, 'AMPS'] = ';'.join(ampliconcounts)
        variant_table.loc[row_i, 'VAF'] = rawvaf

# end vcf fetch

variant_table.to_csv(sys.stdout, sep='\t')
